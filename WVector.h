/*
  WVector.h - Vector library for Wiring & Arduino
  Copyright (c) 2009 Hernando Barragan.  All right reserved.
...
class Vector {
  public:
    ...
    inline boolean add( const Element& obj ) { addElement(obj);  return true; }
    void addElement( const Element& obj );
    inline void clear() { removeAllElements(); }
...
{
  // check for valid index 
  static char dummy_writable_element;
  if(index >= _size || !_data) {
    dummy_writable_element = 0;
    return dummy_writable_element;
  }
